...
"""
FILE ROLE: Local FastAPI AI scoring service used by the Hirely Symfony forum website.
FILE: web/ai_service/app.py
"""

"""
LOGIC INDEX:
- API framework: FastAPI app with /health and /score endpoints.
- Data models: ScoreRequest, ReasonsResponse, ScoreResponse (Pydantic).
- Core feature blocks: relevance scoring, quality scoring, duplicate scoring cache.
- Integration role: called by the Symfony forum moderation flow and compatible with the desktop app contract.
"""
...
except Exception as import_error:  # pragma: no cover - depends on local Python setup
    SentenceTransformer = None  # type: ignore[assignment]
    SENTENCE_TRANSFORMERS_IMPORT_ERROR = str(import_error)
else:
...
# Semantic anchors used by relevance scoring.
CAREER_ANCHORS = {
...
# Request payload expected from the Symfony website and the desktop moderation client.
...
# Structured reasons returned per scoring axis.
...
# Response payload consumed by the Symfony forum AI client.
...
@lru_cache(maxsize=1)
def get_model() -> Any:
    # Lazy-load once; this keeps process startup faster.
    if SentenceTransformer is None:
        raise HTTPException(
            status_code=503,
            detail=(
                "Python AI dependency sentence-transformers is not installed. "
                "Run setup_python_ai.bat on Windows or setup_python_ai.sh on Linux/macOS."
            ),
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Python AI model unavailable: {str(exc)[:180]}",
        ) from exc


@lru_cache(maxsize=1)
def get_anchor_embeddings() -> tuple[dict[str, list[float]], dict[str, list[float]]]:
    # Precompute normalized anchor embeddings once and reuse.
    model = get_model()
...
@app.get("/health")
def health() -> dict:
    # Lightweight endpoint used to verify sidecar availability.
    dependency_ready = SentenceTransformer is not None
    ...
    return {
        ...
        "model": MODEL_NAME,
        "dependencies": {
            "sentence_transformers": "available" if dependency_ready else "missing",
            "error": SENTENCE_TRANSFORMERS_IMPORT_ERROR,
        },
        "setup": "Run setup_python_ai.bat or setup_python_ai.sh if dependencies are missing.",
    }


@app.post("/score", response_model=ScoreResponse)
def score(req: ScoreRequest) -> ScoreResponse:
    # Main API endpoint called by the Symfony forum moderation flow.
...
def score_relevance(embedding: list[float], content_type: str) -> tuple[float, str, list[str]]:
    # Compare content against career/off-topic anchors to compute relevance.
...
def score_quality(text: str, content_type: str) -> tuple[float, list[str]]:
    # Heuristic quality score for spam-like and low-signal patterns.
    score = 1.0
    reasons: list[str] = []
...
def score_duplicate_and_remember(
    ...
    # Duplicate score against recent in-memory embeddings, then cache current item.
...
def cosine(a: list[float], b: list[float]) -> float:
    # Embeddings are normalized; cosine is dot product here.
...
def caps_ratio(text: str) -> float:
    # Uppercase ratio used as quality signal.
...
def token_diversity(text: str) -> float:
    # Unique/total token ratio used as quality signal.
