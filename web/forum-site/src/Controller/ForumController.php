...
class ForumController extends AbstractController
{
    ...
    public function index(Request $request, ForumPostRepository $postRepository, PaginatorInterface $paginator): Response
    {
        ...
        $posts = $paginator->paginate(
            $postRepository->createFeedQueryBuilder(
        ...
        $pageItems = $posts->getItems();
        if (is_array($pageItems)) {
            $postRepository->applyComputedMetrics($pageItems, false, $currentUser);
        }
    ...
    public function showPost(int $id, Request $request, ForumPostRepository $postRepository, ForumCommentRepository $commentRepository, PaginatorInterface $paginator): Response
    {
        ...
        $comments = $paginator->paginate(
            $commentRepository->createForPostQueryBuilder($post, $includeHidden, $commentSort, $currentUser),
    ...
    ): Response
    {
        ...
        $comments = $paginator->paginate(
            $commentRepository->createForPostQueryBuilder($post, $this->isGranted('ROLE_ADMIN'), $commentSort, $currentUser),
            $commentPage,
            8,
            [PaginatorInterface::PAGE_PARAMETER_NAME => 'commentPage']
        );
