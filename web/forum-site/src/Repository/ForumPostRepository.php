...
class ForumPostRepository extends ServiceEntityRepository
{
    ...
    {
        $qb = $this->createFeedQueryBuilder($search, $sort, $tag);
    ...
    public function createFeedQueryBuilder(?string $search, string $sort, ?string $tag = null): QueryBuilder
    {
    ...
    public function findByAuthorForProfile(User $author, bool $includeHidden, string $sort = 'new', ?User $currentUser = null, int $limit = 30): array
    {
        $qb = $this->createQueryBuilder('p')
            ->leftJoin('p.author', 'a')
            ->addSelect('a')
    ...
    private function applySort(QueryBuilder $qb, string $sort): void
    {
        ...
        if (in_array($sort, ['liked', 'most-liked', 'most_liked'], true)) {
            $qb
                ->addSelect('(SELECT COUNT(postLike.id) FROM ' . ForumInteraction::class . ' postLike WHERE postLike.targetType = :postLikeTarget AND postLike.interactionType = :postLikeType AND postLike.targetId = p.id) AS HIDDEN postLikeTotal')
                ->setParameter('postLikeTarget', ForumInteraction::TARGET_POST)
                ->setParameter('postLikeType', ForumInteraction::TYPE_LIKE)
                ->addOrderBy('postLikeTotal', 'DESC')
                ->addOrderBy('p.createdAt', 'DESC');
