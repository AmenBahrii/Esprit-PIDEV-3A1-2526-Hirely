...
class AdminForumController extends AbstractController
{
    ...
    public function dashboard(
        ...
        $posts = $paginator->paginate(
            $postRepository->findAdminFeed(is_string($search) ? $search : null, $status, $sort, $this->getCurrentUserEntity()),
    ...
    public function comments(
        ...
        $comments = $paginator->paginate(
            $commentRepository->findAdminFeed(is_string($search) ? $search : null, $status, $sort, $this->getCurrentUserEntity()),
    ...
    public function showPost(ForumPost $post, Request $request, ForumCommentRepository $commentRepository, PaginatorInterface $paginator): Response
    {
        ...
        $comment = (new ForumComment())
            ->setStatus('PENDING');
        $comments = $paginator->paginate(
            $commentRepository->findForPost($post, true, $commentSort, $this->getCurrentUserEntity()),
            $request->query->getInt('commentPage', 1),
            10,
            [PaginatorInterface::PAGE_PARAMETER_NAME => 'commentPage']
        );
    ...
    public function auditLatest(
        ...
        $auditedPosts = 0;
        foreach ($postRepository->findLatestForAudit(250) as $post) {
        ...
        $auditedComments = 0;
        foreach ($commentRepository->findLatestForAudit(250) as $comment) {
        ...
        $this->addFlash('success', sprintf(
            'AI audit completed for pending content: %d posts and %d comments analyzed.',
            $auditedPosts,
            $auditedComments
        ));
