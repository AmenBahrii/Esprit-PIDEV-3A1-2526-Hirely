...
final class ModerationNoteFormatter
{
    ...
    public function buildSimpleAiNote(ModerationReport $report): string
    {
        ...
        $reasonSummary = $this->summarizeReasons($report->getReasons());
        if ($reasonSummary !== '') {
            $note .= ' | Reasons=' . $reasonSummary;
        }
    ...
    private function summarizeReasons(array $reasons): string
    {
        $clean = array_values(array_filter(array_map(
            static fn (string $reason): string => trim(preg_replace('/\s+/', ' ', $reason) ?? ''),
            $reasons
        )));
        ...
        $summary = implode('; ', array_slice($clean, 0, 2));

        return strlen($summary) <= 140 ? $summary : substr($summary, 0, 137) . '...';
    }
}
