<?php
// Teacher standalone reverse engineering workflow, adapted for this Symfony app.
require_once __DIR__ . '/../vendor/autoload.php';

/**
 * Read DB credentials from DATABASE_URL when available to stay aligned with Symfony config.
 * Fallback matches the shared Hirely local database setup.
 */
$databaseUrl = getenv('DATABASE_URL') ?: 'mysql://root:@127.0.0.1:3306/hirely?serverVersion=8.0&charset=utf8mb4';
$parts = parse_url($databaseUrl);

$dbHost = $parts['host'] ?? '127.0.0.1';
$dbName = isset($parts['path']) ? ltrim($parts['path'], '/') : 'hirely';
$dbUser = $parts['user'] ?? 'root';
$dbPass = $parts['pass'] ?? '';
$dbPort = isset($parts['port']) ? (int) $parts['port'] : 3306;

parse_str($parts['query'] ?? '', $dbQuery);
if (isset($dbQuery['dbname']) && $dbQuery['dbname'] !== '') {
    $dbName = $dbQuery['dbname'];
}

// Keep raw generated output for audit (do not overwrite cleaned entities under src/Entity).
$namespace = 'App\\Entity';
$outputDir = __DIR__ . '/../docs/reverse-engineer/raw-entities';
